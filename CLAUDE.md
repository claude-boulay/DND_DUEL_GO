...
- [x] **Luck reroll audit, prompted by a direct user question ("does it still work, including haggling and duel coin/dice rolls?")** — three-part finding, one real gap fixed:
  ...
  - **Coin toss / dice roll as a duel card effect** (e.g. `Duel.TossCoin`/`Duel.DiceRoll` in a card's Lua script): confirmed NOT implemented — `MessageType` has no `TOSS_COIN`(32)/`TOSS_DICE`(33) entry, so such a prompt would fall through to `describePendingPrompt`'s generic `'unhandled'` case (doesn't crash the duel, `applyMessages` stores any AWAITING message type as `pendingPrompt` regardless) but has no route to actually respond — the duel would soft-lock there. Deliberately NOT built this session: it needs the same live-engine wire-format verification as every other prompt type here, plus a real design call the user should make (does a card's OWN coin toss get to be Luck-reroll-able, thematically consistent with this app's D&D-ified rules but a genuine departure from strict OCG balance) — flagged to the user as optional follow-up work rather than attempted speculatively.
